...
# hyper parameters
number_of_uwb = 4
dic_size = number_of_uwb  # RNN input size (one hot size)
...
x_data= []
y_data= []
file = open('data_sample.txt')
for i in range(10):
    line = file.readline()[:-2].split(' ')
    line_x=[]
    line_y=[]
    for j in range(4):
...
#
X = tf.placeholder(tf.float32, [None, sequence_length, dic_size])  # X data
...
weights = tf.ones([batch_size, sequence_length,hidden_size])
print( outputs.shape)
print( Y.shape)
print( weights.shape)
